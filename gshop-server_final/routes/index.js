...
//================================
//请求热门城市接口
router.get('/hot_citys', function(req, res){

  setTimeout(function () {
    const data = require('../data/hot_citys.json')
    res.send({code: 0, data})
    console.log(data)
  }, 300)
...
//请求字母分类城市接口
router.get('/letter_citys', function(req, res){
  setTimeout(function () {
    const data = require('../data/letter_citys.json')
    res.send({code: 0, data})
    console.log(data)
  }, 300)
...
router.get('/recommend_category_list', function(req, res){
   ...
   console.log(city,types)

   setTimeout(function(){
    const data = require('../data/recommend_list.json')
    var newdata = data.find(function(item, index){//之前没有将过滤的数组复制给另一个变量，一直有bug
